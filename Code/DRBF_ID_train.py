...
from robot_input import DataForDRBF_ID
import parameters as param
from Deep_RBF_network_ID import DRBF_ID
...
images_train,images_test,n_train,n_test=DataForDRBF_ID(batch_size,
                                                   param.p_train)
...
with tf.variable_scope('DRBF_ID') as scope:
    train_step,err_tarin_tf,y_train,_,_=DRBF_ID(images_train,rbf_tf)
    scope.reuse_variables()
    _,err_test_tf,y_test,w_rbf,weights=DRBF_ID(images_test,rbf_tf)
#===define train_step,test and train error===
...
#===training===
err_test=[];
saver.restore(sess, rootdir+'/trained_DRBF_ID.ckpt')
...
save_path = saver.save(sess, rootdir+'/trained_DRBF_ID.ckpt')
#===training===
...
x=im[0];y_pre=im[1];er=im[2]

i=15;
plt.close('all')
plt.figure();pylab.imshow(x[i,:,:]);
plt.figure();pylab.imshow(y_pred[i,:,:])
plt.figure();pylab.imshow(y_pre[i,:,:]-y_pred[i,:,:],cmap='gray')
plt.figure();pylab.imshow(y_pre[i,:,:]-x[i,:,:],cmap='gray')
plt.figure();pylab.imshow(y_pre[i,:,:],cmap='gray')
...
for i in range(n_batch_test):
    ...
    y_GT.append(im[1])
    e_m.append(im[2])
y_GT=np.concatenate(y_GT,0).reshape([100*n_batch_test,64,64])
e_m=np.concatenate(e_m,0).reshape([100*n_batch_test,64,64])

m_y_GT=np.tile(np.mean(y_GT,0),[y_GT.shape[0],1,1])
Syy=np.sum(np.square(y_GT-m_y_GT)*e_m)/(100*n_batch_test*64*64)
R2=1-(RSS/Syy)
#===compute R2===
