...
from robot_input import DataForSPN
...
images_train,images_test,n_train,n_test=DataForSPN(batch_size,
                                                   param.p_train,method)
n_batch_test=n_test/param.TEST_BATCH_SIZE
#===load data and create tf queue=== 

#===define train_step,test and train error===
n_input=3*param.N_command+32
n_state=32
with tf.variable_scope('SPN') as scope:
...
#===training===
err_test=[];
saver.restore(sess, rootdir+'/trained_SPN_BR_OF.ckpt')
...
for epoch in range(20000): 
    ...
    sess.run(train_step)
save_path = saver.save(sess, rootdir+'/trained_SPN_BR_OF.ckpt')
#===training===
...
y_GT=np.concatenate(y_GT,0).reshape([1800,32])
