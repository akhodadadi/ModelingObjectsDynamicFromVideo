...
from robot_input import DataForDRBF_OF
...
images_train,images_test,n_train,n_test=DataForDRBF_OF(batch_size,
                                                   param.p_train)
...
with tf.variable_scope('DRBF') as scope:
    train_step,err_tarin_tf,y_train,_,_=DRBF_OF(images_train,rbf_tf)
    scope.reuse_variables()
    _,err_test_tf,y_test,w_rbf,weights=DRBF_OF(images_test,rbf_tf)
#===define train_step,test and train error===
...
save_path = saver.save(sess, rootdir+'/trained_DRBF_OF.ckpt')
#===training===
